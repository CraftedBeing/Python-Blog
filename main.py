...
# Context Processor to Inject Gravatar
...
def register():
    ...
    if form.validate_on_submit():
        ...
        existing_user = existing_user = User.query.filter(User.email == email).first()
        if existing_user:
        ...
        login_user(new_user)
        # flash('Registration Successfull! Now Logged In.', 'success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            return redirect(url_for('get_all_posts', username= user.name)) # Redirect to secrets dashboard

        else:
            flash('Credentials Mismatch')
            return redirect(url_for('login'))
